#!/usr/bin/env bash
#
# Tmux session manager for running step-2 grid search in the background
#
# This script creates and manages a tmux session for running single-step
# optimization on step-1 artifact circuits. It reuses the AsyncSingleStepRunner
...
#     BQSKIT_NUM_WORKERS=8 ./scripts/run_step2_grid_search_tmux.sh create
#     BQSKIT_WORKER_FRACTION=0.25 ./scripts/run_step2_grid_search_tmux.sh create
#
# Usage:
#   ./scripts/run_step2_grid_search_tmux.sh create           # Create and start search
#   ./scripts/run_step2_grid_search_tmux.sh create --attach   # Create and attach immediately
#   ./scripts/run_step2_grid_search_tmux.sh create --force    # Recreate (kill existing first)
#   ./scripts/run_step2_grid_search_tmux.sh create --rerun    # Rerun all optimizers (keeps history)
#   ./scripts/run_step2_grid_search_tmux.sh create --rerun-optimizers wisq_rules wisq_bqskit
#   ./scripts/run_step2_grid_search_tmux.sh status            # Check session status
#   ./scripts/run_step2_grid_search_tmux.sh attach            # Attach to session
#   ./scripts/run_step2_grid_search_tmux.sh kill              # Kill session
#   ./scripts/run_step2_grid_search_tmux.sh logs              # Tail the log file
#   ./scripts/run_step2_grid_search_tmux.sh db                # Show database stats
...
# Session name
SESSION_NAME="step2-grid-search"

# Output directories
OUTPUT_DIR="$REPO_ROOT/reports/step2_search"
...
STEP1_DATABASE_PATH="${STEP1_DATABASE_PATH:-$REPO_ROOT/data/trajectories.db}"

# Artifact directories
ARTIFACT_INPUT_DIR="${ARTIFACT_INPUT_DIR:-$REPO_ROOT/data/artifacts}"
ARTIFACT_OUTPUT_DIR="${ARTIFACT_OUTPUT_DIR:-$REPO_ROOT/data/artifacts_step2}"
...
show_status() {
    echo "Step-2 Grid Search Status"
    echo "========================="
    echo ""
...
main() {
    ...
    case "${1:-}" in
        ...
        db)
            ...
            ;;
        *)
            echo "Step-2 Grid Search Tmux Session Manager"
            echo "======================================="
            echo ""
