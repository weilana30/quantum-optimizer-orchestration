#!/usr/bin/env bash
#
# Tmux session manager for running step-2 grid search on GUOQ step-1 artifacts
#
# This runs 2-step chaining: takes step-1 optimized circuits and applies
# all 5 optimizers again. For the GUOQ benchmark comparison.
#
# Resource Limiting:
...
#
# Usage:
#   ./scripts/run_guoq_step2_tmux.sh create           # Create and start search
#   ./scripts/run_guoq_step2_tmux.sh create --attach   # Create and attach immediately
#   ./scripts/run_guoq_step2_tmux.sh create --force    # Recreate (kill existing first)
#   ./scripts/run_guoq_step2_tmux.sh status            # Check session status
#   ./scripts/run_guoq_step2_tmux.sh attach            # Attach to session
#   ./scripts/run_guoq_step2_tmux.sh kill              # Kill session
#   ./scripts/run_guoq_step2_tmux.sh logs              # Tail the log file
#   ./scripts/run_guoq_step2_tmux.sh db                # Show database stats
...
# Session name
SESSION_NAME="guoq-step2-grid-search"

# Output directories
OUTPUT_DIR="$REPO_ROOT/reports/guoq_step2_search"
...
# Database paths
DATABASE_PATH="${DATABASE_PATH:-$REPO_ROOT/data/trajectories_guoq_step2.db}"
STEP1_DATABASE_PATH="${STEP1_DATABASE_PATH:-$REPO_ROOT/data/trajectories_guoq.db}"

# Artifact directories
ARTIFACT_INPUT_DIR="${ARTIFACT_INPUT_DIR:-$REPO_ROOT/data/artifacts_guoq}"
ARTIFACT_OUTPUT_DIR="${ARTIFACT_OUTPUT_DIR:-$REPO_ROOT/data/artifacts_guoq_step2}"
...
create_session() {
    ...
    # Verify step-1 DB exists
    if [ ! -f "$STEP1_DATABASE_PATH" ]; then
        echo "Error: Step-1 database not found: $STEP1_DATABASE_PATH"
        echo "Run step-1 grid search first: ./scripts/run_guoq_step1_tmux.sh create"
        exit 1
    fi

    # Verify step-1 artifacts exist
    if [ ! -d "$ARTIFACT_INPUT_DIR" ]; then
        echo "Error: Step-1 artifacts not found: $ARTIFACT_INPUT_DIR"
        echo "Run step-1 grid search first: ./scripts/run_guoq_step1_tmux.sh create"
        exit 1
    fi
    ...
    echo "  Step-2 database:   $DATABASE_PATH"
    echo "  Step-1 database:   $STEP1_DATABASE_PATH"
    ...
    tmux new-session -d -s "$SESSION_NAME" -c "$REPO_ROOT" \
        "export PYTHONWARNINGS='ignore:One or more sample arguments is too small'; \
         echo '=== GUOQ Step-2 Grid Search (Chaining) ==='; \
...
show_status() {
    echo "GUOQ Step-2 Grid Search Status"
    echo "==============================="
    echo ""
...
main() {
    ...
    case "${1:-}" in
        ...
        db)
            ...
            ;;
        *)
            echo "GUOQ Step-2 Grid Search (Chaining) Tmux Session Manager"
            echo "========================================================"
            echo ""
            echo "Runs 2-step optimization chains on GUOQ ibmnew step-1 artifacts."
            echo "Takes step-1 optimized circuits and applies all 5 optimizers again."
            echo ""
            echo "Usage: $0 <command> [args]"
            echo ""
            echo "Commands:"
            echo "  create [flags]    Create and start the step-2 grid search session"
            ...
            echo "  - Import step-1 artifacts from: $ARTIFACT_INPUT_DIR"
            echo "  - Look up categories from:      $STEP1_DATABASE_PATH"
            echo "  - Store results in:             $DATABASE_PATH"
            echo "  - Save step-2 artifacts to:     $ARTIFACT_OUTPUT_DIR"
            echo "  - Log to:                       $LOG_DIR/"
            echo ""
